...
class RNN(nn.Module) :
    def __init__(self, input_size, hidden_size, num_layers=1, dropout=0) :
        ...
        # block_2 FC Layers
        self.fc_channels = [hidden_size, 2 ** 10, 2 ** 10, 2 ** 10, 11]

        self.fc_in = nn.Linear(1024, 1024)
        self.fc_1 = nn.Linear(self.fc_channels[0], self.fc_channels[1])
        self.fc_2 = nn.Linear(self.fc_channels[1], self.fc_channels[2])
        self.fc_3 = nn.Linear(self.fc_channels[2], self.fc_channels[4])
        self.fc_4 = nn.Linear(self.fc_channels[3], self.fc_channels[4])
        self.relu = nn.ReLU()
        self.sig = nn.Sigmoid()
