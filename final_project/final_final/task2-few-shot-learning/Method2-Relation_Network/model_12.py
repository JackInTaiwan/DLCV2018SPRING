...
class RelationNet(nn.Module):
    def __init__(self):
        ...
        score_channel = [feature_channel[-1]*2, 256, 256, 128]
        self.conv_score = nn.Sequential(
            self.conv( score_channel[0], score_channel[1], 3),
            nn.MaxPool2d(kernel_size= 2, stride= 2),
            self.conv( score_channel[1], score_channel[2], 3),
            nn.MaxPool2d(kernel_size= 2, stride= 2),
            self.conv(score_channel[2], score_channel[3], 3)
        )

        fc_channel = [score_channel[-1]*2*2, 128, 1]
