...
class RelationNet(nn.Module):
    def __init__(self):
        ...
        feature_channel = [3, 128, 256, 256, 128]
        ...
        score_channel = [feature_channel[-1]*2, 128, 64]
        ...
        fc_channel = [score_channel[-1]*2*2, 64, 1]
...
def test():
    print('testing')
    rn = RelationNet()
    s = torch.zeros(20,2,3,32,32)
    q = torch.zeros(1,3,32,32)
    out = rn(s,q)
    print(out.shape)

def train_test():
    rn = RelationNet()
    s = torch.zeros(5,5,3,32,32)
    q = torch.zeros(5,3,3,32,32)
    score = rn.train(s,q)
    print(score.shape)
