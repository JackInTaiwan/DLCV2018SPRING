...
try :
    from hw4.VAE.utils import pic_to_npy
except :
...
if __name__ == "__main__" :
    ...
    parser.add_argument("-i", action="store", type=int, default=False)
    parser.add_argument("-f", action="store", type=str)
    parser.add_argument("-l", action="store", type=int)


    print ("======== Starting converting pictures ========")
    name = "{}_data_{}".format(parser.parse_args().m, parser.parse_args().i) if parser.parse_args().i else "{}_data".format(parser.parse_args().m)
